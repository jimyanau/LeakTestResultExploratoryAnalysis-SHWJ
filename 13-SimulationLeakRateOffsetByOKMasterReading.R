...
#Keep only the OK Master values which within spec limit
dt.MC.OKMaster <- dt.AirDecay.MC.Master[dt.AirDecay.MC.Master$part_id == "XBA1601290101A23" & Result == "PASS" ,  ]

dt.WP.OKMaster <- dt.AirDecay.WP.Master[dt.AirDecay.WP.Master$part_id == "XBA1601290101A23" & Result == "PASS"   ,  ]
...
dt.MC.OKMaster$PreviousTestTime[1] <- dt.MC.OKMaster$LeakTestDateTime[1]
dt.MC.OKMaster$PreviousMasterValue[1] <- dt.MC.OKMaster$air_decay_mc[1]
dt.MC.OKMaster$NextTestTime[nrow(dt.MC.OKMaster)] <- dt.MC.OKMaster$LeakTestDateTime[nrow(dt.MC.OKMaster)]
dt.MC.OKMaster$NextMasterValue[nrow(dt.MC.OKMaster)] <- dt.MC.OKMaster$air_decay_mc[nrow(dt.MC.OKMaster)]
...
dt.AirDecay.MC.NoMaster.sub$CurrentMasterValue <- dt.MC.OKMaster$air_decay_mc[ID=dt.AirDecay.MC.NoMaster.sub$MasterDataID]
dt.AirDecay.MC.NoMaster.sub$MasterOffset <- as.numeric(difftime(dt.AirDecay.MC.NoMaster.sub$LeakTestDateTime,dt.AirDecay.MC.NoMaster.sub$PreviousMasterTime,units="days"))*(dt.AirDecay.MC.NoMaster.sub$CurrentMasterValue - dt.AirDecay.MC.NoMaster.sub$PreviousMasterValue)/as.numeric(difftime(dt.AirDecay.MC.NoMaster.sub$CurrentMasterTime,dt.AirDecay.MC.NoMaster.sub$PreviousMasterTime,units="days")) + dt.AirDecay.MC.NoMaster.sub$PreviousMasterValue
...
# Load data without go through long calculation time
dt.AirDecay.MC.NoMaster.sub <- readRDS("DataOutput/dt.AirDecay.MC.OffsetSimulation.RDS")
...
# #Copy to clipboard
write.table(dt.sum.MC.NoMaster.sub, "clipboard", sep="\t", row.names=FALSE)



#make summary by original test result
dt.sum.MC.NoMaster.sub2 <- dt.AirDecay.MC.NoMaster.sub %>%
                            group_by(Result, OffsetResult) %>%
                            summarize(Qty = n(),
                                      OriginalNGQty = sum(Result=="FAIL"),
                                      OriginalRejectPercent = (sum(Result=="FAIL") / Qty)*100,
                                      OffsetNGQty = sum(OffsetResult=="FAIL"),
...
write.table(dt.sum.MC.NoMaster.sub2, "clipboard", sep="\t", row.names=FALSE)



# subset data which passed previously and failed in offset test
dt.test1 <- dt.AirDecay.MC.NoMaster.sub[ Result=="PASS" & OffsetResult == "FAIL",]
